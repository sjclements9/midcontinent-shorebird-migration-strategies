...
hugo <- birds[which(birds$species=='HUGO'),]

max_stop <- function(x){max(x$label)}

brd <- list()
ids <- unique(hugo$id)
for (i in 1:length(ids)){
  brd[[i]] <- hugo[which(hugo$id==ids[i]),]
...
# for 2, 27 <- 444 (missed that)
brd[[2]]$label[which(brd[[2]]$label==27)] <- 444


lapply(brd, max_stop)
# brd 2,9,20,11,12,13,14

brd2 <- brd[c(2, 9:14)]
brd <- brd2
length(brd) #7 of them

# 

brd[[1]]$label[which(brd[[1]]$label==25)] <- 444 # doing this bc of data gap

#max_date <- function(x){max(x$DateTime)}

# just do for hugo for now..
...
for (i in 1:nrow(stop_tabled)){
  ...
  tbl$label <- unique(tmp$label)#[2:length(unique(tmp$label))]
  ...
  for (j in 1:nrow(tbl)){ # anything else you want to calculate by individual stopover you put in hear
    ...
    # step lengths - this is with amt package bc that was how jason did it
...
# 1 case where there was a weird thing happening producing NAs, this gets rid of it
stop_tables[[2]]
stop_tabled$mean_stop_step[2] <- mean(stop_tables[[2]]$mean_step[c(1:8,10)])
stop_tabled$sd_stop_step[2] <- sd(stop_tables[[2]]$mean_step[c(1:8, 10)])
...
stop_tabled$species <- "HUGO"
stop_tabled

stop_tabled$travel_to_stopover <- stop_tabled$mig_dist_km/stop_tabled$total_stop_duration


saveRDS(stop_tabled$travel_to_stopover, file='hugo_travel_to_stopover.rds')
saveRDS(stop_tabled$total_stop_duration, file='hugo_total_stop_duration.rds')
...
stopover_id_map(brd[[4]])

write.csv(stop_tabled, 'D:\\shorebirds\\analysis_code\\migration_strategies\\metric_results\\hugo_mig_metrics_030322_2hr.csv')

hugo_for_analysis <- bind_rows(brd)

write.csv(hugo_for_analysis, 'D:\\shorebirds\\analysis_code\\migration_strategies\\filtered_data\\final_data_full_migrations\\hugo_full_mig_final.csv')
